---
name: session-report
description: Genera informes de sesion educativos en docs/INFORME_SESION.md y, sobre todo, dicta el comportamiento pedagogico de la sesion: NO tomar decisiones tecnicas por el usuario, explicar conceptos como a un principiante, y presentar opciones con pros/contras antes de actuar. Use when the user is a beginner learning to program and asks to "guardar cambios", "informe de sesion", "explicame que hicimos", "eres muy principiante", "no decidas por mi", o cuando el usuario pida documentar el trabajo de la sesion actual.
---

# Session Report - Comportamiento Educativo y Documentación de Sesión
...
2. **Formato del informe de sesión** que se escribe al final en `docs/INFORME_SESION.md`.
...
Al final de cada sesión de trabajo, actualizar `docs/INFORME_SESION.md` con un nuevo bloque siguiendo este formato. El informe es la **memoria del proyecto** y la **herramienta de aprendizaje** del usuario.
...
y por qué. Esto es tan educativo como lo que SÍ se hizo. Ej: "No usamos Chart.js
porque el requerimiento era 7 barras, no ameritaba una dependencia externa".]
...
- **Actualizar también** `CONTEXT.md` si cambió: el stack, las decisiones de diseño, los endpoints, la estructura de archivos, los errores comunes, o el estado del proyecto.
...
- [ ] ¿El `INFORME_SESION.md` está actualizado y sigue el formato de esta skill?
- [ ] ¿El `CONTEXT.md` está actualizado si cambió algo estructural?
- [ ] ¿Hay **secretos** en el `git status`? (verificar con `grep`)
- [ ] ¿Los archivos de config local (`.env`, `.vscode/`, `farmacia.db`) están en `.gitignore`?
...
*Skill creada el 2026-06-04 para formalizar la práctica de informes educativos
+ comportamiento pedagógico. El usuario es principiante y quiere aprender, no
solo recibir código.*
